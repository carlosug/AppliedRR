---
title: 'Applied Research: Descriptive with R'
...
By the end of this assignment(s), we should be able to:
...
- Create your own functions. (bonus)
...
```{r}
mydat1 <- read_xlsx('Workshop Statistics_ descriptives .xlsx')
...
Alrighty, the data contains 74 observations and 8 different variables. Firstly,  the name of the variables seems to be very large and it requires to be re-named in we want to make this dataset ready for further explorations. In addition, we can notice that the amount of empty responses and -99 are considerably high. We can also see that all of variables, except `favourite pet` variable, are considered `numeric` variable. Indeed, it might be worth to convert favourite pet into a `dummy` variable. It is therefore time to wrangling the data, and clean it a little bit!
...
Last three columns are inconveniently named. Using `names()` with specific position will change the name of these columns. Be sure to select properly the elements of the vector. 

The name of column `favourite pet: cat(=1) or dog (=2)` is very long. We can make it shorter!
Rename it to `favPet`. The position of this column is 6. Then the command will be as following:
...
#### Exercise 1: Check/listlist the variable names in our data set. Have the variable names of column 6 through 8 been successfully changed?
...
It looks like ‘Unknown’ values within tutor variable are missing values. But they still have to be identified as such.

#### Exercise 2: Could you convert all these records into `NA`?
_Intructions_: `[mydat == -99]` should be now `[mydat == 'unknown']`
...
Dummy variables (or binary variables) are a specific type of categorical variable. They have only two values (0 = an event hasn’t occurred; 1 = an event occurred). Dummy variables are commonly used in statistical analyses and in more simple descriptive statistics. A dummy column is one which has a value of one when a categorical event occurs and a zero when it doesn’t occur. Indeed, we want to modify the variable `favPet` to a dummy variable. 
Specifically, we want the variable `favPet` to say whether we have seen a dog (=1) or not (=0; thus a cat). In this case, value 0 takes when is ‘cat’. Otherwise value 1.

Here you can see how to convert the value 1 for cat too value 0 when it is a cat.
```{r}
...
#### Exercise 5: Can you please return the record number 12 of the column `favPet` from `mydat`? You should see whether she/he answered pet or dog.
...
Filtering for one favourite pet and one year:
```{r}
...
Instructions: 1. create an object `mydatkoen` 
              2. assign `<- filter(mydat, mydat$(here tutor column) == '')`
...
So roughly we can say that the age and length average for students along 2019 and 2020 is 21 year old and 1,70 mts, recepectively.
...
#### Exercise 12: Can you find the mean and median of length variable across 2020? 
Use the following instruction for the exercise:
`summarize(filter(mydat, year == ""),`
          `mean2020 = mean("", na.rm = TRUE),`
           `median2020 = median("", na.rm = TRUE))`
...
You've seen how to find the mean and median `age` and `length` across a set of observations.

To sum up, using `summary` as before, R return as output the statistics of all the dataset. Instead, `filter` and `summarize` compute a specific measurement in a specific subset of your data.

Great! We already consolidated concepts from last workshop: mean, variance and standard deviation. 
We have used R functions and they are cool. Now it its time for making our graphs!
However, for challengers whose want to make their own functions please go ahead and make the bonus section before the visualisation section!
...
#### Exercise 13: Can you create a barplot representing the `tutor names` on the bars?
...
#### Exercise 14: Could you please explain me what this point portriated on the graph means?
...
#### Exercise 15: Could you plot the descriptive of the `length` variable?
...
#### Exercise 16: Now that you have explored both variables, age and length, how do they differ in terms of data distribution?
...
#### Exercise 17: Which favourite pet did the oldest person choose in the dataset?


#### Exercise 18: As in the last graph, could you create another boxplot that shows the distribution of `favPet` responses and `lenght` variable?
...
#### Exercise 19: Can you create a graph that shows the relationship between the variables `age` and `length` in mydat `datset`? Give some interpretations about the nature of the relationship between these two variables.
...
### From preprocessing data of research paper:Human Origin, the evolution of modern human in Europe
...
iris.uci1 <- read_excel("irisdataset.xlsx")
library(datasets)
data(iris)
summary(iris.uci)
str(iris.uci)
